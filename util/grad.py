...
def compute_numeric_gradient(f, x, dLdf=None, h=1e-3):
    """
    ...
    for i in range(flat_x.shape[0]):
        oldval = flat_x[i]  # Store the original value
...
def rel_error(x, y, eps=1e-10):
    ...
    top = np.abs(x - y).max()
    bot = (np.abs(x) + np.abs(y)).clip(0, None).max()
    return top / bot